...
namespace async_coro
{
	...
	void scheduler::continue_execution_impl(base_handle& handle_impl) {
		...
		handle_impl._state = handle_impl._handle.done() ? coroutine_state::finished : coroutine_state::suspended;
		if (handle_impl._state == coroutine_state::finished && handle_impl._parent && handle_impl._parent->_state == coroutine_state::suspended) {
			// wake up parent coroutine
			continue_execution(*handle_impl._parent);
		}
	}

	void scheduler::add_coroutine(base_handle& handle_impl, execution_thread thread) {
	...
	void scheduler::change_thread(base_handle& handle_impl, execution_thread thread) {
		ASYNC_CORO_ASSERT(handle_impl._scheduler == this);
		ASYNC_CORO_ASSERT(!is_current_thread_fits(thread));

		if (thread == execution_thread::main_thread) {
			std::unique_lock lock {_task_mutex};
			_update_tasks_syncronized.push_back([handle_base = &handle_impl](auto& thiz) {
