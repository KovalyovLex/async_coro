...
class zstd_decompress::impl {
 public:
  ...
  static std::unique_ptr<impl> make_impl(zstd::window_log window_log) {
    ...
    return std::unique_ptr<impl>(reinterpret_cast<impl*>(dctx));  // NOLINT(*reinterpret-cast)
  }

  static std::unique_ptr<impl> make_impl_with_dict(std::span<const std::byte> dictionary, zstd::window_log window_log) {
...
bool zstd_decompress::update_stream(std::span<const std::byte>& data_in, std::span<std::byte>& data_out) noexcept {
  ...
  const size_t err = ZSTD_decompressStream(_impl->as_context(), &output, &input);
...
bool zstd_decompress::flush_impl(std::span<const std::byte>& data_in, std::span<std::byte>& data_out, bool& finished) noexcept {
  ...
  const size_t remaining = ZSTD_decompressStream(_impl->as_context(), &output, &input);
